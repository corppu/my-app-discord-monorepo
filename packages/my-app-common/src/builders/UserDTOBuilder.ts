...
export class UserDTOBuilder {
  ...
  build(): UserDTO {
    const { id, discordId, username, displayName, email, createdAt, updatedAt } = this.dto;
    ...
    if (!discordId) {
      throw new ValidationError("validation.required", "discordId", this.locale);
    }
    if (!DISCORD_ID_REGEX.test(discordId)) {
      throw new ValidationError("validation.invalidDiscordId", "discordId", this.locale);
    }
    ...
    if (!USERNAME_REGEX.test(username)) {
      throw new ValidationError("validation.invalidUsername", "username", this.locale);
    }

    if (!displayName) {
      throw new ValidationError("validation.required", "displayName", this.locale);
    }
    if (displayName.length < 1 || displayName.length > 32) {
      throw new ValidationError("validation.invalidDisplayName", "displayName", this.locale);
    }

    if (email !== undefined && email !== "") {
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        throw new ValidationError("validation.invalidEmail", "email", this.locale);
      }
    }
